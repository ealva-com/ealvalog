...
public interface Logger {
   ...
   *
   * @param marker {@link Marker} to be used with every log, unless overridden per method, or null
   *               to remove any current {@link Marker}
   */
  ...
  /**
   * Set an optional LogLevel. If null the nearest ancestor LogLevel will be used as the level
   * for this logger
   * <p>
  ...
  /**
   * Gets the level set for this logger or, if that it has not been set, get the nearest ancestor
   * log level
   *
   * @return the log level at which this logger is operating
   */
  @NotNull LogLevel getEffectiveLogLevel();
  ...
  /**
   * Determine if a log call at this {@link LogLevel} will result in an actual log statement.
   * Typically this is only a level check, unless the Logger instance contains a {@link Marker}. In
   * that case the contained Marker is also checked to promote fast short-circuiting
   *
   * @param level the level to test, one of {@link LogLevel#TRACE}, {@link LogLevel#DEBUG},
   *              {@link LogLevel#INFO}, {@link LogLevel#WARN}, {@link LogLevel#ERROR}, {@link LogLevel#CRITICAL}
   *
  ...
  /**
   * Will a log at this {@link LogLevel}, with the given (optional) {@link Marker} and (optional)
   * {@link Throwable}, result in an actual log statement
   *
   * @param level     the level to test, one of {@link LogLevel#TRACE}, {@link LogLevel#DEBUG},
   *                  {@link LogLevel#INFO}, {@link
   ...
   * ({@code stackDepth}). This method's primary use is for this logging framework and it's not
   * expected client's would typically use this method.
   * <p>
   * This method has a {@link Marker} parameter which will override any {@link Marker} in the
   * Logger itself
   *
   ...
   * @param formatArgs any formatting arguments if {@code msg} is a printf style format string
   *                   (see {@link Formatter}
   */
